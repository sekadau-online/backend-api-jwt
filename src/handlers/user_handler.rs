...
use crate::schemas::user_schema::{
    UserResponseSchema, UserStoreRequestSchema, UserUpdateRequestSchema,
};
...
pub async fn index(
    ...
    match (users_result, total_result) {
        (Ok(users), Ok(total)) => {
            let response = ApiResponse::success_with_data(
        ...
        (Err(e), _) | (_, Err(e)) => {
            let response = ApiResponse::error_with_data(
                "Database error",
                json!({ "error": "Failed to fetch users", "details": e.to_string() }),
            );
...
pub async fn show(
    ...
    match user_result {
        ...
        Ok(None) => {
            let response =
                ApiResponse::error_with_data("Not Found", json!({ "error": "User not found" }));
            Err((StatusCode::NOT_FOUND, Json(response)))
        }
        Err(e) => {
            let response = ApiResponse::error_with_data(
                "Database error",
                json!({ "error": "Failed to fetch user", "details": e.to_string() }),
            );
            Err((StatusCode::INTERNAL_SERVER_ERROR, Json(response)))
        }
    }
...
pub async fn update(
    ...
    match result {
        ...
        Err(e) => {
            let response = ApiResponse::error_with_data(
                "Database error",
                json!({ "error": "Failed to update user", "details": e.to_string() }),
            );
            Err((StatusCode::INTERNAL_SERVER_ERROR, Json(response)))
        }
    }
...
pub async fn destroy(
    ...
    match result {
        ...
        Err(e) => {
            let response = ApiResponse::error_with_data(
                "Database error",
                json!({ "error": "Failed to delete user", "details": e.to_string() }),
            );
            Err((StatusCode::INTERNAL_SERVER_ERROR, Json(response)))
        }
    }
