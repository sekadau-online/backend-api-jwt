...
async fn user_delete_flow() {
    ...
    // generate token
    let token = backend_api_jwt::utils::jwt::generate_jwt_token(user_id)
        .await
    ...
    // No auth -> 401 (not 405)
    let res_unauth = client.delete(&url).send().await.expect("request failed");
