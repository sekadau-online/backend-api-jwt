...
async fn proxy_trusted_uses_header_source() {
    ...
    unsafe { std::env::set_var("TRUSTED_PROXIES", "127.0.0.1/32"); }
    ...
    let xs = res.headers().get("x-key-source").and_then(|v| v.to_str().ok()).unwrap_or("");
    assert!(xs == "extension" || xs == "cf-connecting-ip", "unexpected x-key-source: {}", xs);
...
async fn proxy_not_trusted_uses_header_directly() {
    ...
    assert!(xs == "cf-connecting-ip" || xs == "peer", "unexpected x-key-source: {}", xs);
