...
async fn cors_wildcard_allows_any_origin() {
    ...
    let database_url = match std::env::var("DATABASE_URL") {
        Ok(v) => v,
        Err(_) => {
            eprintln!("Skipping test: set DATABASE_URL");
            return;
        }
    };
    ...
    if !res.status().is_success() {
        panic!(
            "Preflight failed: status={} headers={:?}",
            res.status(),
            res.headers()
        );
    }
...
async fn cors_specific_origins() {
    ...
    unsafe {
        std::env::set_var(
            ...
            "http://allowed.example.com,https://foo.bar",
        );
    }
    ...
    if let Some(val) = allowed2 {
        assert!(
            val == "*" || val == "http://allowed.example.com",
            "Disallowed origin unexpectedly allowed: headers={:?}",
            res2.headers()
        );
    }
}
