...
async fn user_store_flow() {
    ...
    let test_db = "db_backend_api_jwt_test_store";
    ...
    let admin_id = insert_res.last_insert_id() as i64;
    ...
    let res_unauth = client
        .post(&url)
        .json(&json!({"name": "ShouldNotWork", "email": "noauth@example.com", "password": "password"}))
        .send()
        .await
    ...
    let res = client
        ...
        .json(&json!({"name": "New User", "email": "newuser@example.com", "password": "password"}))
        .send()
        .await
        .expect("request failed");
    ...
    assert_eq!(created["email"].as_str().unwrap_or(""), "newuser@example.com");
